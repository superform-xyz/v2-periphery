...
# This copies fresh bytecode from out/ to generated-bytecode/ folder
log "INFO" "Regenerating bytecode artifacts for VNET deployment..."
if ! ./script/run/tooling/regenerate_bytecode.sh; then
    log "ERROR" "Failed to regenerate bytecode artifacts"
    exit 1
fi

# Copy generated bytecode to locked-bytecode-dev for VNET deployment
# VNET uses locked-bytecode-dev (env=1) which will be freshly generated
log "INFO" "Copying generated bytecode to locked-bytecode-dev..."
mkdir -p script/locked-bytecode-dev
cp script/generated-bytecode/*.json script/locked-bytecode-dev/
log "INFO" "Bytecode artifacts copied to locked-bytecode-dev successfully"
...
filter_allowed_periphery_contracts() {
    ...
    local allowed_contracts=("SuperGovernor" "SuperVault" "SuperVaultAggregator" "SuperVaultStrategy" "SuperVaultEscrow" "ECDSAPPSOracle" "FixedPriceOracle" "SuperOracle" "SuperOracleL2" "SuperBank")
...
update_latest_file() {
    ...
    for network in 1 8453 10; do
        ...
        if [ -z "$network_exists" ] || [ "$network_exists" = "null" ]; then
            ...
            local fixed_price_oracle=$(echo "$filtered_contracts" | jq -r '.FixedPriceOracle // empty')
            local super_oracle=$(echo "$filtered_contracts" | jq -r '.SuperOracle // empty')
            local super_oracle_l2=$(echo "$filtered_contracts" | jq -r '.SuperOracleL2 // empty')
            local super_bank=$(echo "$filtered_contracts" | jq -r '.SuperBank // empty')
            ...
            if [ -n "$fixed_price_oracle" ] && [ "$fixed_price_oracle" != "empty" ]; then
                existing_contracts=$(echo "$existing_contracts" | jq --arg addr "$fixed_price_oracle" '.FixedPriceOracle = $addr')
                log "INFO" "Updated FixedPriceOracle: $fixed_price_oracle"
            fi

            if [ -n "$super_oracle" ] && [ "$super_oracle" != "empty" ]; then
                existing_contracts=$(echo "$existing_contracts" | jq --arg addr "$super_oracle" '.SuperOracle = $addr')
                log "INFO" "Updated SuperOracle: $super_oracle"
            fi

            if [ -n "$super_oracle_l2" ] && [ "$super_oracle_l2" != "empty" ]; then
                existing_contracts=$(echo "$existing_contracts" | jq --arg addr "$super_oracle_l2" '.SuperOracleL2 = $addr')
                log "INFO" "Updated SuperOracleL2: $super_oracle_l2"
            fi

            if [ -n "$super_bank" ] && [ "$super_bank" != "empty" ]; then
                existing_contracts=$(echo "$existing_contracts" | jq --arg addr "$super_bank" '.SuperBank = $addr')
                log "INFO" "Updated SuperBank: $super_bank"
            fi
