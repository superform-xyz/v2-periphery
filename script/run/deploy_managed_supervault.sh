...
###################################################################################
# Deploy ManagedSuperVault Script
###################################################################################
# Description:
#   Deploys the ManagedSuperVault contract family - the vault/controller/escrow
#   implementations and the ManagedSuperVaultAggregator sibling factory - then
#   registers the aggregator in SuperGovernor.
#
# Usage:
#   ./deploy_managed_supervault.sh <environment> <mode> [account] [chain_id]
#
#   Parameters:
#     environment: "prod" or "staging"
#     mode: "simulate", "execute", "check", or "register"
#     account: Account name (required for execute/register, e.g., "v2-supervaults")
#     chain_id: Optional chain ID (default: deploys on all supported chains)
#
#   Modes:
#     simulate  Dry-run the family deployment (no broadcast)
#     execute   Deploy the trio + aggregator deterministically
#     check     Report deployment + registration status (read-only)
#     register  Register the aggregator under MANAGED_SUPER_VAULT_AGGREGATOR.
#               MUST use the SUPER_GOVERNOR_ROLE account. Run after `execute`.
#
# Examples:
#   ./deploy_managed_supervault.sh staging check
#   ./deploy_managed_supervault.sh staging simulate
#   ./deploy_managed_supervault.sh staging execute v2-supervaults
#   ./deploy_managed_supervault.sh staging execute v2-supervaults 8453
#   ./deploy_managed_supervault.sh staging register <governor-account>
#   ./deploy_managed_supervault.sh prod execute v2-supervaults
#
# Prerequisites:
#   - 1Password CLI configured for RPC URL access
#   - For execute/register mode: Foundry account configured
#   - SuperGovernor must already be deployed on target chains
#   - Locked bytecode must be generated (run regenerate_bytecode.sh first)
#
# Post-Deployment Steps:
#   1. Register the aggregator: `register` mode with the SUPER_GOVERNOR_ROLE account
#      (sets MANAGED_SUPER_VAULT_AGGREGATOR in the SuperGovernor address registry).
#   2. Optionally deploy the session-key executor via
#      deploy_managed_supervault_executor.sh, then add it as a secondary manager.
#
...
Arguments:
    ...
    mode         Mode: "simulate", "execute", "check", or "register" (required)
    account      Account name (required for execute/register, e.g., "v2-supervaults")
    chain_id     Optional chain ID to deploy on a specific chain

Examples:
    # Check deployment + registration status on all staging chains
    $0 staging check
    ...
    # Register the aggregator (SUPER_GOVERNOR_ROLE account)
    $0 staging register <governor-account>
...
validate_mode() {
    local mode=$1
    if [ "$mode" != "simulate" ] && [ "$mode" != "execute" ] && [ "$mode" != "check" ] && [ "$mode" != "register" ]; then
        log "ERROR" "Invalid mode: $mode"
        log "ERROR" "Must be 'simulate', 'execute', 'check', or 'register'"
        exit 1
    fi
}
...
deploy_on_chain() {
    ...
    if [ "$mode" = "execute" ]; then
        ...
        # Registration broadcasts a setAddress tx; no contract verification needed
        BROADCAST_FLAG="--broadcast"
        ACCOUNT_FLAG="--account $account"
        log "INFO" "Mode: Register (will broadcast using SUPER_GOVERNOR_ROLE account: $account)"
    ...
    forge_cmd+=" script/DeployManagedSuperVault.s.sol:DeployManagedSuperVault"

    if [ "$mode" = "check" ]; then
        forge_cmd+=" --sig 'runCheck(uint256,uint64,string)' $env $chain_id \"\""
    elif [ "$mode" = "register" ]; then
        forge_cmd+=" --sig 'runRegister(uint256,uint64,string)' $env $chain_id \"\""
    else
...
main() {
    ...
    # Validate account for execute / register mode (both broadcast)
    if [ "$mode" = "execute" ] || [ "$mode" = "register" ]; then
        if [ -z "$account" ]; then
            log "ERROR" "Account name is required for $mode mode"
            log "ERROR" "Usage: $0 $environment $mode <account_name>"
            exit 1
        fi
    ...
    log "INFO" "============================================"
    log "INFO" "Deploy ManagedSuperVault family"
    log "INFO" "============================================"
    log "INFO" "Environment: $environment (env=$env)"
    log "INFO" "Mode: $mode"
    log "INFO" "Admin: $ADMIN (SUPER_GOVERNOR_ADDRESS)"
    log "INFO" "Note: SuperGovernor address is resolved from core deterministic deployment"
    ...
    log "INFO" "============================================"
    log "INFO" "ManagedSuperVault deployment completed successfully!"
    if [ "$mode" = "execute" ]; then
        log "INFO" "NEXT: run '$0 $environment register <governor-account>' to register the aggregator."
    fi
    log "INFO" "============================================"
}
