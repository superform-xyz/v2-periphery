...
###################################################################################
# ProposeUpkeepPause Emergency Script
###################################################################################
# Description:
#   Emergency script to propose pausing upkeep payments on SuperGovernor.
#   This script is mainnet-only and should be used to disable upkeep payments
#   in emergency situations.
#
# Usage:
#   ./propose_upkeep_pause.sh <environment> <mode> [account]
#
#   Parameters:
...
#   ./propose_upkeep_pause.sh staging simulate
#
#   # Execute on mainnet staging
#   ./propose_upkeep_pause.sh staging execute v2-supervaults
#
#   # Simulate on mainnet prod
#   ./propose_upkeep_pause.sh prod simulate
#
#   # Execute on mainnet prod
#   ./propose_upkeep_pause.sh prod execute v2-supervaults
#
# Prerequisites:
#   - For execute mode: Foundry account with SUPER_GOVERNOR_ROLE permissions
#   - 1Password CLI configured for RPC URL access
#
# Note:
#   - This script only runs on mainnet (chain ID 1)
#   - This is a two-step process:
#     1. This script proposes the change (starts timelock)
#     2. After timelock, execute_upkeep_pause.sh must be run
#
...
main() {
    ...
    log "INFO" "============================================"
    log "INFO" "ProposeUpkeepPause Script"
    log "INFO" "============================================"
    ...
    forge_cmd+=" script/ProposeUpkeepPause.s.sol:ProposeUpkeepPause"
    ...
    if [ $exit_code -eq 0 ]; then
        log "INFO" "============================================"
        log "INFO" "ProposeUpkeepPause completed successfully!"
        log "INFO" "============================================"
        log "INFO" ""
        log "INFO" "IMPORTANT: This is only a proposal."
        log "INFO" "After the timelock period expires, run:"
        log "INFO" "  ./execute_upkeep_pause.sh $environment execute $account"
        log "INFO" "============================================"
    else
        log "ERROR" "============================================"
        log "ERROR" "ProposeUpkeepPause FAILED with exit code: $exit_code"
        log "ERROR" "============================================"
        exit $exit_code
    fi
}
