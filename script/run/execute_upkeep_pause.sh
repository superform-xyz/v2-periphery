...
###################################################################################
# ExecuteUpkeepPause Script
###################################################################################
# Description:
#   Script to execute a previously proposed upkeep payments pause on SuperGovernor.
#   This script is mainnet-only and should be run after the timelock period
#   has elapsed following a propose_upkeep_pause.sh call.
#
# Usage:
#   ./execute_upkeep_pause.sh <environment> <mode> [account]
#
#   Parameters:
...
#   ./execute_upkeep_pause.sh staging simulate
#
#   # Execute on mainnet staging
#   ./execute_upkeep_pause.sh staging execute v2-supervaults
#
#   # Simulate on mainnet prod
#   ./execute_upkeep_pause.sh prod simulate
#
#   # Execute on mainnet prod
#   ./execute_upkeep_pause.sh prod execute v2-supervaults
#
# Prerequisites:
#   - A pending upkeep payments change must exist (from propose_upkeep_pause.sh)
#   - The timelock period must have elapsed
#   - For execute mode: Foundry account with permissions
...
#     1. propose_upkeep_pause.sh proposes the change (starts timelock)
#     2. This script executes the change after timelock expires
#
...
main() {
    ...
    case "$environment" in
        prod)
            env=0
            salt="$PRODUCTION_SALT_NAMESPACE"
            ;;
        staging)
            env=2
            salt="$STAGING_SALT_NAMESPACE"
            ;;
    esac
    ...
    log "INFO" "============================================"
    log "INFO" "ExecuteUpkeepPause Script"
    log "INFO" "============================================"
    ...
    forge_cmd+=" script/UpkeepPauseExecute.s.sol:UpkeepPauseExecute"
    ...
    if [ $exit_code -eq 0 ]; then
        log "INFO" "============================================"
        log "INFO" "ExecuteUpkeepPause completed successfully!"
        log "INFO" "============================================"
        log "INFO" ""
        log "INFO" "Upkeep payments have been disabled."
        log "INFO" "============================================"
    else
        log "ERROR" "============================================"
        log "ERROR" "ExecuteUpkeepPause FAILED with exit code: $exit_code"
        log "ERROR" "============================================"
        exit $exit_code
    fi
}
