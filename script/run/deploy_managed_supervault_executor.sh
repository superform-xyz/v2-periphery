...
###################################################################################
# Deploy ManagedSuperVaultExecutor Script
###################################################################################
# Description:
#   Deploys ManagedSuperVaultExecutor - session key delegation contract for secondary
#   manager functions on Managed Vault controllers.
#
# Usage:
#   ./deploy_managed_supervault_executor.sh <environment> <mode> [account] [chain_id]
#
#   Parameters:
...
#   ./deploy_managed_supervault_executor.sh staging check
#
#   # Check deployment status on specific chain
#   ./deploy_managed_supervault_executor.sh staging check "" 8453
#
#   # Simulate deployment on all staging chains
#   ./deploy_managed_supervault_executor.sh staging simulate
#
#   # Simulate deployment on specific chain
#   ./deploy_managed_supervault_executor.sh staging simulate "" 8453
#
#   # Execute deployment on all staging chains
#   ./deploy_managed_supervault_executor.sh staging execute v2-supervaults
#
#   # Execute deployment on specific chain
#   ./deploy_managed_supervault_executor.sh staging execute v2-supervaults 8453
#
#   # Execute deployment on all prod chains
#   ./deploy_managed_supervault_executor.sh prod execute v2-supervaults
#
# Prerequisites:
...
#   1. Add ManagedSuperVaultExecutor as secondary manager on each controller via
#      ManagedSuperVaultAggregator.addSecondaryManager(controller, executorAddr)
#   2. Grant session keys via the primary manager calling
#      ManagedSuperVaultExecutor.grantSessionKey(controller, sessionKey, expiry, perms)
#
...
deploy_on_chain() {
    ...
    forge_cmd+=" script/DeployManagedSuperVaultExecutor.s.sol:DeployManagedSuperVaultExecutor"
...
main() {
    ...
    log "INFO" "============================================"
    log "INFO" "Deploy ManagedSuperVaultExecutor"
    log "INFO" "============================================"
    ...
    log "INFO" "============================================"
    log "INFO" "ManagedSuperVaultExecutor deployment completed successfully!"
    log "INFO" "============================================"
}
