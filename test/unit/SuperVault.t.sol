...
contract SuperVaultTest is PeripheryHelpers {
    ...
    // =============================================================
    // _isPPSNotUpdated AND Condition Tests
    // =============================================================
    // These tests verify the behavior of the updated _isPPSNotUpdated function:
    // return (elapsed > aggMaxStaleness) && (elapsed > ppsExpiration);
    //
    // PPS is considered "not updated" (expired) only when BOTH conditions are true.
    // This prevents DoS scenarios where a short ppsExpiration could block deposits
    // even when the oracle network is functioning properly within maxStaleness.
    // =============================================================

    /// @notice Test DoS prevention: ppsExpiration < maxStaleness
    /// @dev When ppsExpiration is shorter than maxStaleness, operations should NOT revert
    ///      if elapsed time only exceeds ppsExpiration but is within maxStaleness
    ///      This is the key DoS prevention scenario - oracle can still push updates within maxStaleness
    function test_isPPSNotUpdated_DoSPrevention_PpsExpirationLessThanMaxStaleness() public {
        // Create a vault with maxStaleness = 7 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "DoS Prevention Vault",
                symbol: "DPV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
                maxStaleness: 7 days, // 7 days maxStaleness
                feeConfig: ISuperVaultStrategy.FeeConfig({
                    performanceFeeBps: 0,
                    managementFeeBps: 0,
                    recipient: manager
        ...
        // Set ppsExpiration to 1 day (shorter than maxStaleness)
        // Storage slot 12 is ppsExpiration based on the storage layout
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(1 days)));

        // Verify the ppsExpiration was set correctly
        assertEq(testStrategy.ppsExpiration(), 1 days, "ppsExpiration should be 1 day");

        // Verify maxStaleness is 7 days
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 7 days, "maxStaleness should be 7 days");

        // Setup: Give user assets and approval
        address testUser = _deployAccount(0xABC, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward 2 days (exceeds ppsExpiration of 1 day, but within maxStaleness of 7 days)
        vm.warp(block.timestamp + 2 days);

        // Verify elapsed > ppsExpiration but < maxStaleness
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertGt(elapsed, 1 days, "Elapsed should exceed ppsExpiration");
        assertLt(elapsed, 7 days, "Elapsed should be within maxStaleness");

        // Test: Deposit should NOT revert because only one condition is true
        // (elapsed > ppsExpiration) = true, (elapsed > maxStaleness) = false
        // AND condition requires BOTH to be true
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);

        // Verify deposit succeeded
        assertGt(testVault.balanceOf(testUser), 0, "User should have received shares");
    }

    /// @notice Test ppsExpiration enforcement when ppsExpiration > maxStaleness
    /// @dev When maxStaleness is shorter than ppsExpiration, operations should NOT revert
    ///      if elapsed time only exceeds maxStaleness but is within ppsExpiration
    function test_isPPSNotUpdated_PpsExpirationGreaterThanMaxStaleness_OnlyMaxStalenessExceeded() public {
        // Create a vault with maxStaleness = 1 day (short)
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Short Staleness Vault",
                symbol: "SSV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
                maxStaleness: 1 days, // 1 day maxStaleness
                feeConfig: ISuperVaultStrategy.FeeConfig({
                    performanceFeeBps: 0,
                    managementFeeBps: 0,
                    recipient: manager
        ...
        // Set ppsExpiration to 7 days (longer than maxStaleness)
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(7 days)));

        // Verify the ppsExpiration was set correctly
        assertEq(testStrategy.ppsExpiration(), 7 days, "ppsExpiration should be 7 days");

        // Verify maxStaleness is 1 day
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 1 days, "maxStaleness should be 1 day");
        ...
        // Fast forward 2 days (exceeds maxStaleness of 1 day, but within ppsExpiration of 7 days)
        vm.warp(block.timestamp + 2 days);

        // Verify elapsed > maxStaleness but < ppsExpiration
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertGt(elapsed, 1 days, "Elapsed should exceed maxStaleness");
        assertLt(elapsed, 7 days, "Elapsed should be within ppsExpiration");

        // Test: Deposit should NOT revert because only one condition is true
        // (elapsed > ppsExpiration) = false, (elapsed > maxStaleness) = true
        // AND condition requires BOTH to be true
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
    ...
    /// @notice Test edge case where ppsExpiration equals maxStaleness
    /// @dev When both thresholds are equal and exceeded, operations should revert
    function test_isPPSNotUpdated_EqualThresholds_BothExceeded_Reverts() public {
        // Create a vault with maxStaleness = 1 day
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Equal Thresholds Vault",
                symbol: "ETV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Set ppsExpiration to 1 day (same as maxStaleness)
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(1 days)));
        ...
        // Verify maxStaleness is also 1 day
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 1 days, "maxStaleness should be 1 day");

        // Setup: Give user assets and approval
        address testUser = _deployAccount(0x123, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward exactly 1 day + 1 second (exceeds both thresholds)
        vm.warp(block.timestamp + 1 days + 1);

        // Verify elapsed > both thresholds
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertGt(elapsed, 1 days, "Elapsed should exceed both thresholds");

        // Test: Deposit should revert with PPS_EXPIRED because BOTH conditions are true
        vm.prank(testUser);
        vm.expectRevert(ISuperVaultStrategy.PPS_EXPIRED.selector);
        testVault.deposit(100e18, testUser);
    }

    /// @notice Test when both thresholds are exceeded (ppsExpiration < maxStaleness)
    /// @dev Should revert when elapsed exceeds BOTH thresholds
    function test_isPPSNotUpdated_BothThresholdsExceeded_Reverts() public {
        // Create a vault with maxStaleness = 3 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Both Exceeded Vault",
                symbol: "BEV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Set ppsExpiration to 2 days
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(2 days)));

        // Verify configuration
        assertEq(testStrategy.ppsExpiration(), 2 days, "ppsExpiration should be 2 days");
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 3 days, "maxStaleness should be 3 days");

        // Setup: Give user assets and approval
        address testUser = _deployAccount(0x456, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward 4 days (exceeds both: 2 days ppsExpiration and 3 days maxStaleness)
        vm.warp(block.timestamp + 4 days);

        // Verify elapsed > both thresholds
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertGt(elapsed, 2 days, "Elapsed should exceed ppsExpiration");
        assertGt(elapsed, 3 days, "Elapsed should exceed maxStaleness");
    ...
    /// @notice Test when neither threshold is exceeded
    /// @dev Should NOT revert when elapsed is within both thresholds
    function test_isPPSNotUpdated_NeitherThresholdExceeded_Succeeds() public {
        // Create a vault with maxStaleness = 3 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Neither Exceeded Vault",
                symbol: "NEV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Setup: Give user assets and approval
        address testUser = _deployAccount(0x789, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward 1 day (within both: 2 days ppsExpiration and 3 days maxStaleness)
        vm.warp(block.timestamp + 1 days);

        // Verify elapsed < both thresholds
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertLt(elapsed, 2 days, "Elapsed should be within ppsExpiration");
        assertLt(elapsed, 3 days, "Elapsed should be within maxStaleness");

        // Test: Deposit should succeed because neither condition is true
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
    ...
    /// @notice Test boundary: elapsed exactly equals ppsExpiration (not exceeded)
    /// @dev When elapsed == ppsExpiration, condition (elapsed > ppsExpiration) is FALSE
    function test_isPPSNotUpdated_ElapsedExactlyEqualsPpsExpiration_Succeeds() public {
        // Create a vault with maxStaleness = 3 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
        ...
        // Setup: Give user assets and approval
        address testUser = _deployAccount(0xAAA, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward exactly 1 day (equals ppsExpiration, not exceeded)
        vm.warp(block.timestamp + 1 days);

        // Verify elapsed == ppsExpiration
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertEq(elapsed, 1 days, "Elapsed should equal ppsExpiration");

        // Test: Deposit should succeed because (elapsed > ppsExpiration) is FALSE
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
    ...
    /// @notice Test boundary: elapsed exactly equals maxStaleness (not exceeded)
    /// @dev When elapsed == maxStaleness, condition (elapsed > maxStaleness) is FALSE
    function test_isPPSNotUpdated_ElapsedExactlyEqualsMaxStaleness_Succeeds() public {
        // Create a vault with maxStaleness = 2 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Max Staleness Boundary Vault",
                symbol: "MSBV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Set ppsExpiration to 1 day (so ppsExpiration < maxStaleness)
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(1 days)));

        // Verify configuration
        assertEq(testStrategy.ppsExpiration(), 1 days, "ppsExpiration should be 1 day");
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 2 days, "maxStaleness should be 2 days");

        // Setup: Give user assets and approval
        address testUser = _deployAccount(0xBBB, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward exactly 2 days (equals maxStaleness, exceeds ppsExpiration)
        vm.warp(block.timestamp + 2 days);

        // Verify elapsed conditions
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);
        uint256 elapsed = block.timestamp - lastUpdate;
        assertEq(elapsed, 2 days, "Elapsed should equal maxStaleness");
        assertGt(elapsed, 1 days, "Elapsed should exceed ppsExpiration");

        // Test: Deposit should succeed because (elapsed > maxStaleness) is FALSE
        // Even though (elapsed > ppsExpiration) is TRUE, AND requires BOTH
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
    ...
    /// @notice Test mint operation with DoS prevention
    /// @dev Verify mint also benefits from the AND condition
    function test_isPPSNotUpdated_Mint_DoSPrevention() public {
        // Create a vault with maxStaleness = 7 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Mint DoS Prevention Vault",
                symbol: "MDPV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Setup: Give user assets and approval
        address testUser = _deployAccount(0xCCC, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward 2 days (exceeds ppsExpiration but within maxStaleness)
        vm.warp(block.timestamp + 2 days);

        // Test: Mint should succeed
        vm.prank(testUser);
        testVault.mint(100e18, testUser);
    ...
    /// @notice Test requestRedeem operation with DoS prevention
    /// @dev Verify requestRedeem also benefits from the AND condition
    function test_isPPSNotUpdated_RequestRedeem_DoSPrevention() public {
        // Create a vault with maxStaleness = 7 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "RequestRedeem DoS Prevention Vault",
                symbol: "RRDPV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Setup: Give user assets, deposit first to get shares
        address testUser = _deployAccount(0xDDD, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.startPrank(testUser);
        asset.approve(vaultAddr, 10_000e18);
        testVault.deposit(1000e18, testUser);
        uint256 shares = testVault.balanceOf(testUser);
        ...
        // Test: RequestRedeem should succeed
        vm.prank(testUser);
        testVault.requestRedeem(shares / 2, testUser, testUser);

        // Verify request was made
        uint256 pending = testVault.pendingRedeemRequest(0, testUser);
        assertEq(pending, shares / 2, "Pending redeem should match requested amount");
    }

    /// @notice Test that all operations revert when both thresholds are exceeded
    /// @dev Comprehensive test for deposit, mint, and requestRedeem
    function test_isPPSNotUpdated_AllOperationsRevert_WhenBothExceeded() public {
        // Create a vault with maxStaleness = 2 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "All Operations Revert Vault",
                symbol: "AORV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Setup: Give user assets and deposit before expiry
        address testUser = _deployAccount(0xEEE, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.startPrank(testUser);
        asset.approve(vaultAddr, 10_000e18);
        testVault.deposit(1000e18, testUser);
        ...
        // Fast forward 3 days (exceeds both: 1 day ppsExpiration and 2 days maxStaleness)
        ...
        // Test: Mint should revert
        vm.prank(testUser);
        vm.expectRevert(ISuperVaultStrategy.PPS_EXPIRED.selector);
        testVault.mint(100e18, testUser);

        // Test: RequestRedeem should revert
        vm.prank(testUser);
        vm.expectRevert(ISuperVaultStrategy.PPS_EXPIRED.selector);
        testVault.requestRedeem(shares / 2, testUser, testUser);
    }

    /// @notice Test using managePPSExpiration to set ppsExpiration (full lifecycle)
    /// @dev Tests the proper workflow: propose -> wait for timelock -> execute
    function test_isPPSNotUpdated_WithManagePPSExpiration_FullLifecycle() public {
        // Create a vault with maxStaleness = 7 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Full Lifecycle Vault",
                symbol: "FLV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Default ppsExpiration is 1 day
        assertEq(testStrategy.ppsExpiration(), 1 days, "Default ppsExpiration should be 1 day");

        // Propose new ppsExpiration of 2 hours (short)
        vm.prank(manager);
        testStrategy.managePPSExpiration(ISuperVaultStrategy.PPSExpirationAction.Propose, 2 hours);
        ...
        vm.prank(manager);
        testStrategy.managePPSExpiration(ISuperVaultStrategy.PPSExpirationAction.Execute, 0);

        // Verify ppsExpiration was updated
        assertEq(testStrategy.ppsExpiration(), 2 hours, "ppsExpiration should be updated to 2 hours");

        // Now test the AND condition with new ppsExpiration
        // maxStaleness = 7 days, ppsExpiration = 2 hours

        // Setup: Give user assets
        address testUser = _deployAccount(0xFFF, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Get last update timestamp (which is now after the 1 week timelock)
        uint256 lastUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);

        // Warp 1 day from last update (exceeds 2 hours ppsExpiration, within 7 days maxStaleness)
        vm.warp(lastUpdate + 1 days);

        // Test: Deposit should succeed because only ppsExpiration is exceeded
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
    ...
    /// @notice Test DoS prevention using managePPSExpiration (no vm.store)
    /// @dev Demonstrates the proper governance workflow for ppsExpiration changes
    ///      and verifies DoS prevention with the AND condition
    function test_isPPSNotUpdated_DoSPrevention_UsingManagePPSExpiration() public {
        // Create a vault with maxStaleness = 7 days
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "DoS Prevention Governance Vault",
                symbol: "DPGV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
                maxStaleness: 7 days, // Long maxStaleness for oracle network
                feeConfig: ISuperVaultStrategy.FeeConfig({
                    performanceFeeBps: 0,
                    managementFeeBps: 0,
                    recipient: manager
        ...
        // Verify default ppsExpiration is 1 day
        assertEq(testStrategy.ppsExpiration(), 1 days, "Default ppsExpiration should be 1 day");
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 7 days, "maxStaleness should be 7 days");

        // Step 1: Manager proposes a shorter ppsExpiration (1 hour)
        // This simulates a strategist wanting tighter freshness requirements
        vm.prank(manager);
        testStrategy.managePPSExpiration(ISuperVaultStrategy.PPSExpirationAction.Propose, 1 hours);

        // Step 2: Wait for the 1 week timelock to pass
        vm.warp(block.timestamp + 1 weeks + 1);

        // Step 3: Execute the ppsExpiration update
        vm.prank(manager);
        testStrategy.managePPSExpiration(ISuperVaultStrategy.PPSExpirationAction.Execute, 0);

        // Verify ppsExpiration was updated to 1 hour
        assertEq(testStrategy.ppsExpiration(), 1 hours, "ppsExpiration should be 1 hour");

        // Record the last PPS update timestamp (set during execute due to timelock warp)
        uint256 lastPPSUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);

        // Setup: Give user assets and approval
        address testUser = _deployAccount(0xABCD, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // =========== Scenario A: Within ppsExpiration (30 minutes) ===========
        // Both conditions false -> deposit succeeds
        vm.warp(lastPPSUpdate + 30 minutes);

        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
        assertGt(testVault.balanceOf(testUser), 0, "Deposit should succeed within ppsExpiration");

        // =========== Scenario B: Exceeds ppsExpiration (2 hours), within maxStaleness ===========
        // This is the key DoS prevention test!
        // Old behavior: Would revert because elapsed > ppsExpiration
        // New behavior: Does NOT revert because elapsed < maxStaleness
        vm.warp(lastPPSUpdate + 2 hours);

        uint256 balanceBefore = testVault.balanceOf(testUser);
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
        uint256 balanceAfter = testVault.balanceOf(testUser);

        assertGt(balanceAfter, balanceBefore, "Deposit should succeed - DoS prevention active");

        // =========== Scenario C: Well past ppsExpiration (3 days), still within maxStaleness ===========
        vm.warp(lastPPSUpdate + 3 days);

        balanceBefore = testVault.balanceOf(testUser);
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
        balanceAfter = testVault.balanceOf(testUser);

        assertGt(balanceAfter, balanceBefore, "Deposit should succeed - still within maxStaleness");

        // =========== Scenario D: Exactly at maxStaleness boundary (7 days) ===========
        // elapsed == maxStaleness, condition uses >, so should still work
        vm.warp(lastPPSUpdate + 7 days);
        ...
        assertGt(balanceAfter, balanceBefore, "Deposit should succeed at exactly maxStaleness boundary");

        // =========== Scenario E: Exceeds BOTH thresholds (8 days) ===========
        // Now both conditions are true -> deposit should revert
        vm.warp(lastPPSUpdate + 8 days);
    ...
    /// @notice Test that ppsExpiration change via governance also works for reverting case
    /// @dev Uses managePPSExpiration to set a long ppsExpiration, then tests revert when both exceeded
    function test_isPPSNotUpdated_BothExceeded_UsingManagePPSExpiration() public {
        // Create a vault with maxStaleness = 2 days (shorter than default ppsExpiration)
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Both Exceeded Governance Vault",
                symbol: "BEGV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Change ppsExpiration to 3 days (longer than maxStaleness)
        vm.prank(manager);
        testStrategy.managePPSExpiration(ISuperVaultStrategy.PPSExpirationAction.Propose, 3 days);
        ...
        // Verify configuration
        assertEq(testStrategy.ppsExpiration(), 3 days, "ppsExpiration should be 3 days");
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 2 days, "maxStaleness should be 2 days");

        // Record the last PPS update timestamp
        uint256 lastPPSUpdate = superVaultAggregator.getLastUpdateTimestamp(strategyAddr);

        // Setup user
        address testUser = _deployAccount(0xBCDE, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Scenario: 2.5 days elapsed (exceeds maxStaleness but not ppsExpiration)
        // Should succeed because only one condition is true
        vm.warp(lastPPSUpdate + 2 days + 12 hours);

        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
        assertGt(testVault.balanceOf(testUser), 0, "Deposit should succeed - only maxStaleness exceeded");

        // Scenario: 4 days elapsed (exceeds BOTH thresholds)
        // Should revert
        vm.warp(lastPPSUpdate + 4 days);
    ...
    /// @notice Test extreme values: very short ppsExpiration, very long maxStaleness
    /// @dev Simulates production scenario with conservative ppsExpiration but long oracle staleness
    function test_isPPSNotUpdated_ExtremeValues_ShortPpsExpiration_LongMaxStaleness() public {
        // Create a vault with maxStaleness = 7 days (MAX_PPS_EXPIRATION_THRESHOLD)
        vm.prank(manager);
        (address vaultAddr, address strategyAddr,) = superVaultAggregator.createVault(
            ISuperVaultAggregator.VaultCreationParams({
                asset: address(asset),
                name: "Extreme Values Vault",
                symbol: "EVV",
                mainManager: manager,
                secondaryManagers: new address[](0),
                minUpdateInterval: 5,
        ...
        // Set ppsExpiration to minimum (1 minute)
        bytes32 ppsExpirationSlot = bytes32(uint256(12));
        vm.store(strategyAddr, ppsExpirationSlot, bytes32(uint256(1 minutes)));

        // Verify configuration
        assertEq(testStrategy.ppsExpiration(), 1 minutes, "ppsExpiration should be 1 minute");
        assertEq(superVaultAggregator.getMaxStaleness(strategyAddr), 7 days, "maxStaleness should be 7 days");

        // Setup: Give user assets
        address testUser = _deployAccount(0x111, "TestUser");
        deal(address(asset), testUser, 10_000e18);
        vm.prank(testUser);
        asset.approve(vaultAddr, 10_000e18);

        // Fast forward 1 hour (exceeds 1 minute ppsExpiration, far within 7 days maxStaleness)
        vm.warp(block.timestamp + 1 hours);

        // Test: Deposit should succeed
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);
        assertGt(testVault.balanceOf(testUser), 0, "Deposit should succeed");

        // Fast forward to 6 days total (still within maxStaleness)
        vm.warp(block.timestamp + 6 days - 1 hours);

        // Test: Deposit should still succeed
        vm.prank(testUser);
        testVault.deposit(100e18, testUser);

        // Fast forward past 7 days total (exceeds both thresholds now)
        vm.warp(block.timestamp + 2 days);

        // Test: Deposit should now revert
        vm.prank(testUser);
        vm.expectRevert(ISuperVaultStrategy.PPS_EXPIRED.selector);
        testVault.deposit(100e18, testUser);
    }
