...
import { MockSuperHook } from "../../../../test/mocks/MockSuperHook.sol";
import { MockHookTarget } from "../../../../test/mocks/MockHookTarget.sol";
...
contract VaultBankTest is PeripheryHelpers {
    ...
    function test_unlockAsset_Success_WithSP_Transfer() public {
        ...
        // Get the actual SuperPosition address that was created
    ...
    function test_constructor() public {
        ...
        assertEq(
            address(newVaultBank.SUPER_REGISTRY()),
            newRegistry,
            "SUPER_REGISTRY should be set to the provided registry address"
        );
    ...
    function test_batchDistributeRewardsToSuperBank() public {
        ...
        vm.mockCall(
            address(superRegistry), abi.encodeWithSignature("isRelayer(address)", address(this)), abi.encode(true)
        );
