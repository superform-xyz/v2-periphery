...
/// @title SuperBankOpenOceanIntegration
/// @notice Integration tests for ApproveAndSwapOpenOceanHook on Flare (chain 14)
/// @dev Validates production merkle root from superman/deployments/superbank/generated/
///      Tests real swaps on Flare fork using live OpenOcean V4 quotes.
///
/// Run:
///   forge test --match-contract SuperBankOpenOceanIntegration -vvv
contract SuperBankOpenOceanIntegration is Test {
    using Surl for *;
    using strings for *;
    ...
    // ── Flare (14) hooks ──
    address constant FLARE_APPROVE_AND_SWAP_OPENOCEAN_HOOK = 0x5147932fEf0103Af148aB25f073f94Db8B85E76D;
    ...
    // Flare ApproveAndSwapOpenOceanHook: 1 leaf (dstReceiver = SuperBank)
    bytes32 constant FLARE_OPENOCEAN_ROOT = 0x0648eac0fa7524dd1badbb46fab3c42313c100c7f9fe28a9af388e38504e7c7f;
    ...
    /// @notice Verify computed leaf hash matches generated tree for ApproveAndSwapOpenOceanHook on Flare.
    /// @dev inspect() returns abi.encodePacked(dstReceiver)
    ...
    function test_inspectAndVerifyLeaf_approveAndSwapOpenOcean_flare() public pure {
        bytes memory encodedArgs = abi.encodePacked(SUPER_BANK);
        ...
        assertEq(
            ...
            "Leaf hash mismatch for Flare OpenOcean dstReceiver=SuperBank"
        );
    }
    ...
    /// @notice Set production merkle root for ApproveAndSwapOpenOceanHook on Flare and verify.
    function test_setProductionMerkleRoots_openOceanOnFlare() public {
        ...
        console2.log("Flare ApproveAndSwapOpenOceanHook production merkle root set successfully");
    }
    ...
    /// @notice Full integration: swap WFLR→SPRK on Flare via ApproveAndSwapOpenOceanHook with production merkle tree.
    function test_executeHooks_swapWFLRtoSPRK_openOcean_onFlare() public {
        ...
        (bytes memory txData, uint256 minOutAmount) =
            _getOpenOceanSwapData(WFLR, SPRK, swapAmount, SUPER_BANK);

        // Encode ApproveAndSwapOpenOceanHook data
        bytes memory hookData = _encodeOpenOceanHookData(
            WFLR, SPRK, swapAmount, minOutAmount / 2, false, txData
        );

        // Execute with production proof (empty proof for single-leaf tree)
        uint256 sprkBefore = IERC20(SPRK).balanceOf(SUPER_BANK);
        bytes32[] memory emptyProof = new bytes32[](0);

        superBank.executeHooks(
            _buildSingleHookExecutionData(
                FLARE_APPROVE_AND_SWAP_OPENOCEAN_HOOK, hookData, emptyProof
            )
        );
    ...
    {
        // OpenOcean amount param is human-readable (not wei) for the basic API
        // but we use amountDecimals for precision
        string memory url = string.concat(
            OPENOCEAN_SWAP_URL,
            "?chain=14",
    ...
    /// @dev ApproveAndSwapOpenOceanHook data layout:
