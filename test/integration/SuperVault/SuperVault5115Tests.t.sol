...
contract SuperVault5115Tests is BaseSuperVaultTest {
    ...
    function test_SuperVault_5115_ReAllocate5115To4626() public {
        ...
        vars.expectedAssetsOrSharesOut[1] = Mock4626Vault(test11_Allocate_NewYieldSource).convertToShares(vars.assetAmountToReallocateFrom5115); // Expected shares from 4626 deposit
    ...
    function test_SuperVault_5115_ReAllocate5115To4626() public {
        ...
        // deposit 1
        deal(address(asset5115), accountEth, depositAmount);
        _deposit(depositAmount, address(sv5115), address(asset5115));
        _depositFreeAssetsFromSingleAmount5115(depositAmount, address(strategy5115SuperVault), pendleEthenaAddress);

        uint256 shares1 = IERC20(sv5115.share()).balanceOf(accountEth);
        assertGt(shares1, 0, "no shares minted for deposit 1");

        vm.warp(block.timestamp + 4 weeks);
        uint256 ppsBefore = aggregator.getPPS(address(strategy5115SuperVault));
        _updateSuperVaultPPS(address(strategy5115SuperVault), address(sv5115));
        uint256 ppsAfter = aggregator.getPPS(address(strategy5115SuperVault));
        assertGt(ppsAfter, ppsBefore, "pps not greater after 5115 deposit");
        ...
        // initial balances
        uint256 initial5115Balance = pendleEthena.balanceOf(address(strategy5115SuperVault));
        uint256 initialNewVaultBalance = Mock4626Vault(test11_Allocate_NewYieldSource).balanceOf(address(strategy5115SuperVault));
        
        // amount to reallocate (50% of 5115 vault)
        uint256 amountToReallocateFrom5115 = initial5115Balance * 50 / 100;
        uint256 assetAmountToReallocateFrom5115 = pendleEthena.previewRedeem(address(asset5115), amountToReallocateFrom5115);
        
        console2.log("Asset amount to reallocate from 5115 vault:", assetAmountToReallocateFrom5115);
        console2.log("Initial 5115 balance:", initial5115Balance);
        console2.log("Amount to reallocate from 5115:", amountToReallocateFrom5115);
        // Get hook addresses
        address withdraw5115HookAddress = _getHookAddress(ETH, REDEEM_5115_VAULT_HOOK_KEY);
        address deposit4626HookAddress = _getHookAddress(ETH, APPROVE_AND_DEPOSIT_4626_VAULT_HOOK_KEY);

        address[] memory hooksAddresses = new address[](2);
        hooksAddresses[0] = withdraw5115HookAddress;
        hooksAddresses[1] = deposit4626HookAddress;
        ...
        hooksData[1] = _createApproveAndDeposit4626HookData(
            ...
            test11_Allocate_NewYieldSource,
            address(asset5115),
            assetAmountToReallocateFrom5115,
            false,
            address(0),
            0
        );
        ...
        expectedAssetsOrSharesOut[0] = assetAmountToReallocateFrom5115; // Expected assets from 5115 redeem
        expectedAssetsOrSharesOut[1] = Mock4626Vault(test11_Allocate_NewYieldSource).convertToShares(assetAmountToReallocateFrom5115); // Expected shares from 4626 deposit
        ...
        // Verify reallocation results
        uint256 final5115Balance = pendleEthena.balanceOf(address(strategy5115SuperVault));
        uint256 finalNewVaultBalance = Mock4626Vault(test11_Allocate_NewYieldSource).balanceOf(address(strategy5115SuperVault));

        // Assert that 5115 vault balance decreased
        assertApproxEqRel(
            final5115Balance,
            initial5115Balance - amountToReallocateFrom5115,
            0.01e18,
            "5115 vault balance should decrease by reallocated amount"
        );

        // Assert that 4626 vault balance increased
        assertGt(finalNewVaultBalance, initialNewVaultBalance, "4626 vault balance should increase after reallocation");
        
        // Verify total assets are preserved (approximately)
        uint256 totalAssetsAfter = sv5115.totalAssets();
        assertApproxEqRel(
            totalAssetsAfter,
            depositAmount,
            0.05e18,
            "Total assets should be approximately preserved after reallocation"
        );

        console2.log("Final 5115 balance:", final5115Balance);
        console2.log("Final new vault balance:", finalNewVaultBalance);
        console2.log("Total assets after reallocation:", totalAssetsAfter);
    }
    
    function test_SuperVault_5115_ReAllocateFrom4626To5115() public {
        vm.selectFork(FORKS[ETH]);
        _setup5115Vault();

        uint256 depositAmount = 2000e6; // 2000 USDC

        // Initial deposit to 5115 vault
        _deposit(depositAmount, address(sv5115), address(asset5115));
        
        // Allocate to 4626 vaults first (using the test vaults)
        _depositFreeAssetsTo4626FromStrategy5115(depositAmount, address(strategy5115SuperVault), test1_SuperVault_5115_ReAllocateFrom4626To5115_Vault1, test2_SuperVault_5115_ReAllocateFrom4626To5115_Vault2);

        // Verify initial allocation to 4626 vaults
        uint256 vault1Balance = Mock4626Vault(test1_SuperVault_5115_ReAllocateFrom4626To5115_Vault1).balanceOf(address(strategy5115SuperVault));
        uint256 vault2Balance = Mock4626Vault(test2_SuperVault_5115_ReAllocateFrom4626To5115_Vault2).balanceOf(address(strategy5115SuperVault));
        
        assertGt(vault1Balance, 0, "No shares in vault1 before reallocation");
        assertGt(vault2Balance, 0, "No shares in vault2 before reallocation");

        // Get initial 5115 vault balance (should be 0 since we allocated to 4626 vaults)
        uint256 initial5115Balance = pendleEthena.balanceOf(address(strategy5115SuperVault));
        
        // Calculate amounts to reallocate (50% from each 4626 vault)
        uint256 reallocateFromVault1 = vault1Balance / 2;
        uint256 reallocateFromVault2 = vault2Balance / 2;
        
        uint256 assetAmountFromVault1 = Mock4626Vault(test1_SuperVault_5115_ReAllocateFrom4626To5115_Vault1).convertToAssets(reallocateFromVault1);
        uint256 assetAmountFromVault2 = Mock4626Vault(test2_SuperVault_5115_ReAllocateFrom4626To5115_Vault2).convertToAssets(reallocateFromVault2);
        uint256 totalAssetAmount = assetAmountFromVault1 + assetAmountFromVault2;
        ...
        hooksData[0] = _createRedeem4626HookData(
            ...
            reallocateFromVault1,
            false
        );
        expectedAssetsOrSharesOut[0] = Mock4626Vault(test1_SuperVault_5115_ReAllocateFrom4626To5115_Vault1).previewRedeem(reallocateFromVault1);
        argsForProofs[0] = ISuperHookInspector(hooksAddresses[0]).inspect(hooksData[0]);

        // Hook 2: Withdraw from vault2
        ...
        hooksData[1] = _createRedeem4626HookData(
            ...
            address(strategy5115SuperVault),
            reallocateFromVault2,
            false
        );
        expectedAssetsOrSharesOut[1] = Mock4626Vault(test2_SuperVault_5115_ReAllocateFrom4626To5115_Vault2).previewRedeem(reallocateFromVault2);
        argsForProofs[1] = ISuperHookInspector(hooksAddresses[1]).inspect(hooksData[1]);

        // Hook 3: Deposit to 5115 vault (Pendle)
        hooksAddresses[2] = depositHookAddress;
        hooksData[2] = _create5115DepositHookData(
            _getYieldSourceOracleId(bytes32(bytes(ERC5115_YIELD_SOURCE_ORACLE_KEY)), MANAGER),
            pendleEthenaAddress,
            address(asset5115),
            totalAssetAmount,
        ...
        expectedAssetsOrSharesOut[2] = totalAssetAmount; // Expected shares out from 5115 deposit
        argsForProofs[2] = ISuperHookInspector(hooksAddresses[2]).inspect(hooksData[2]);

        // Execute reallocation hooks
        vm.startPrank(MANAGER);
        strategy5115SuperVault.executeHooks(
            ISuperVaultStrategy.ExecuteArgs({
                hooks: hooksAddresses,
        ...
        // Verify reallocation results
        {
            uint256 finalVault1Balance = Mock4626Vault(test1_SuperVault_5115_ReAllocateFrom4626To5115_Vault1).balanceOf(address(strategy5115SuperVault));
            uint256 finalVault2Balance = Mock4626Vault(test2_SuperVault_5115_ReAllocateFrom4626To5115_Vault2).balanceOf(address(strategy5115SuperVault));
            uint256 final5115Balance = pendleEthena.balanceOf(address(strategy5115SuperVault));

            // Assert that 4626 vault balances decreased
            assertApproxEqRel(
                finalVault1Balance,
                vault1Balance - reallocateFromVault1,
                0.01e18,
                "Vault1 balance should decrease by reallocated amount"
            );
            
            assertApproxEqRel(
                finalVault2Balance,
                vault2Balance - reallocateFromVault2,
                0.01e18,
                "Vault2 balance should decrease by reallocated amount"
            );

            // Assert that 5115 vault balance increased
            assertGt(final5115Balance, initial5115Balance, "5115 vault balance should increase after reallocation");
            
            // Verify total assets are preserved (approximately)
            uint256 totalAssetsBefore = sv5115.totalAssets();
            assertApproxEqRel(
                totalAssetsBefore,
                depositAmount,
                0.05e18,
