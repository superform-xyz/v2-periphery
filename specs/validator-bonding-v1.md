...
struct BondRecord {
    ...
    address beneficiary;         // Who receives sUP on unbond (validator or Foundation)
...
| Function | Access | Description |
|----------|--------|-------------|
| `requestUnbond(uint256 amount)` | Operator OR beneficiary | Start unbonding. Partial (remaining >= minimumBond) or full. |
| `executeUnbond()` | Operator OR beneficiary | After `unbondingPeriod`, sends sUP to `beneficiary`. |
| `cancelUnbond()` | Operator OR beneficiary | Cancel pending unbond, re-bond tokens. |

Both the operator and beneficiary can initiate/execute/cancel unbonding. This ensures the Foundation (as beneficiary) can pull back a loan without needing the operator's cooperation.
...
3. If full unbond: status = `Unbonded`, removed from `_operators`
...
**No permanent exclusion status.** Slashing just reduces the bond. Whether the operator can re-bond later is a governance decision — they'd need to `bond()` again AND be re-added to `setValidatorConfig()`. The real gatekeeping is the permissioned validator set in SuperGovernor.
...
event UnbondRequested(address indexed operator, uint256 amount, uint256 executeAfter);
