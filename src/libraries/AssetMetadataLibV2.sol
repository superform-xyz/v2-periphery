...
/// @dev V2: removes the code.length == 0 guard. While Base B20 precompile tokens actually return
///      0xef (1 byte) from eth_getCode (so extcodesize == 1, not 0), removing the guard makes
///      the library future-proof for any precompile-style assets that may have zero or minimal
///      bytecode. The staticcall + returndata length check already handles invalid addresses safely.
library AssetMetadataLib {
     ...
     *      Unlike V1, this does NOT check code.length. Note: Base B20 precompile tokens
     *      return 0xef (1 byte) via eth_getCode so they already pass V1's check too;
     *      removing the guard here is a defensive measure for future precompile variants.
