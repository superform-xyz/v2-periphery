...
import { AssetMetadataLib } from "../libraries/AssetMetadataLibV2.sol";

/// @title SuperVaultStrategyV2
/// @author Superform Labs
/// @notice Strategy implementation for SuperVault that executes strategies.
/// @dev V2 additions: AssetMetadataLibV2 (B20 precompile support), ERC721/ERC1155 receiver callbacks,
///      ERC1271 signature validation, and ERC165 interface detection.
contract SuperVaultStrategyV2 is ISuperVaultStrategy, Initializable, ReentrancyGuardUpgradeable {
    ...
    /// @dev Required for any ERC721 contract that uses _safeMint or safeTransferFrom to send tokens to
    ///      this contract. Note: protocols that use plain _mint (e.g. Uniswap V3 NonfungiblePositionManager)
    ///      do NOT invoke this callback, so onERC721Received is irrelevant for those flows.
    function onERC721Received(address, address, uint256, bytes calldata) external pure returns (bytes4) {
        return IERC721Receiver.onERC721Received.selector;
    }

    /// @notice Allows the contract to receive ERC1155 tokens
    /// @dev Required for hooks that may transfer ERC1155 tokens to the strategy
    function onERC1155Received(address, address, uint256, uint256, bytes calldata) external pure returns (bytes4) {
        return IERC1155Receiver.onERC1155Received.selector;
    }

    /// @notice Allows the contract to receive batches of ERC1155 tokens
    /// @dev Required for hooks that may batch-transfer ERC1155 tokens to the strategy
    ...
    /// @notice ERC1271 signature validation - always valid for governance-approved hooks
    /// @dev The strategy only executes hooks approved via merkle root governance. Any signature
    ///      produced by the strategy is inherently authorized, so we return the magic value unconditionally.
    ...
    function skimPerformanceFee() external nonReentrant {
        ...
        // Rounding: Floor is intentional. Ceil would round the reduction up, understating backing and
        // silently penalizing depositors. Floor slightly overstates backing by at most 1 PPS unit per skim,
        // which self-corrects on the next aggregator PPS update from on-chain balances. This is the
        // standard ERC4626-compliant choice (favor the vault/depositors over the protocol).
