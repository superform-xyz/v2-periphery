...
/// ERC7540. V2 uses AssetMetadataLibV2 which supports protocol-level precompile assets (e.g. Base B20 tokens).
contract SuperVaultV2 is
    Initializable,
    ERC20Upgradeable,
    ISuperVault,
    ...
    EIP712Upgradeable
{
    ...
    function supportsInterface(bytes4 interfaceId) public pure returns (bool) {
        return interfaceId == type(IERC7540Redeem).interfaceId || interfaceId == type(IERC7540CancelRedeem).interfaceId
    ...
    /// @notice Returns the address of the share token (this vault itself, per ERC7575)
    /// @dev Replaces the storage variable from V1 — the value is always address(this), so no slot needed.
