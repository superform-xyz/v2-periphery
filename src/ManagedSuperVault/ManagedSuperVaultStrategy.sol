...
/// @title ManagedSuperVaultStrategy
/// @author Superform Labs
/// @notice Strategy implementation for ManagedSuperVault that executes strategies
/// @dev Fork of SuperVaultStrategy — review by diffing against src/SuperVault/SuperVaultStrategy.sol.
///      Single managed diff: the aggregator is stored at initialize (msg.sender) instead of being
///      resolved from the global SUPER_VAULT_AGGREGATOR registry key, since managed strategies belong
///      to the ManagedSuperVaultAggregator instance.
contract ManagedSuperVaultStrategy is ISuperVaultStrategy, Initializable, ReentrancyGuardUpgradeable {
    ...
    // Managed diff: the aggregator that created this strategy (stored at initialize; the global
    // SUPER_VAULT_AGGREGATOR registry key points at the main-family aggregator, not ours)
    address private _aggregatorAddr;
    ...
    function _getSuperVaultAggregator() internal view returns (ISuperVaultAggregator) {
        ...
        return ISuperVaultAggregator(_aggregatorAddr);
    }
