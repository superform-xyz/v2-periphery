...
/// @notice Managed vault share token implementing ERC4626 with queue-gated synchronous deposits and asynchronous
/// redeems via ERC7540
/// @dev Fork of SuperVault — review by diffing against src/SuperVault/SuperVault.sol. Managed diffs:
///      (1) deposit/mint are gated to the vault's deposit queue — managed vaults price at a manager-attested
///      NAV on a slow cadence, so open synchronous deposits would enable stale-NAV timing arbitrage and
///      bypass the queue's deposit approval policy; (2) the aggregator is stored at initialize (msg.sender)
///      instead of being resolved from the global SUPER_VAULT_AGGREGATOR registry key.
    ...
    EIP712Upgradeable
{
    ...
    /// @notice Thrown when anyone other than the deposit queue calls deposit/mint (managed diff)
    error ONLY_DEPOSIT_QUEUE();
    ...
    // Managed diffs: the only address allowed to deposit/mint, and the creating aggregator
    // (stored at initialize; the global SUPER_VAULT_AGGREGATOR registry key points at the
    // main-family aggregator, not ours)
    ...
    /// @param depositQueue_ The deposit queue address — the only account allowed to deposit/mint
    function initialize(
        address asset_,
        string memory name_,
        ...
        initializer
    {
        /// @dev asset, strategy, escrow, and depositQueue already validated in ManagedSuperVaultAggregator
        ...
        // Managed diff: initialize is only ever called by the creating aggregator (createVault)
        _aggregator = msg.sender;
    ...
    //////////////////////////////////////////////////////////////*/
    /// @inheritdoc IERC4626
    /// @dev Managed diff: only the deposit queue may deposit — open sync deposits at an attested NAV
    ///      would enable stale-NAV timing arbitrage and bypass the queue's approval policy
    function deposit(uint256 assets, address receiver) public override nonReentrant returns (uint256 shares) {
        if (msg.sender != depositQueue) revert ONLY_DEPOSIT_QUEUE();
    ...
    /// @inheritdoc IERC4626
    /// @dev Managed diff: only the deposit queue may mint (see deposit)
    ...
    /// @inheritdoc IERC4626
    /// @dev Managed diff: only the deposit queue may deposit, so report 0 for everyone else
    function maxDeposit(address receiver) public view override returns (uint256) {
        if (receiver != depositQueue) return 0;
        if (!_canAcceptDeposits()) return 0;
        return type(uint256).max;
    }

    /// @inheritdoc IERC4626
    /// @dev Managed diff: only the deposit queue may mint, so report 0 for everyone else
    ...
    /// @return Address of the ManagedSuperVaultAggregator contract
    function _getAggregatorAddress() internal view returns (address) {
        // Managed diff: return the creating aggregator stored at initialize
        return _aggregator;
    }
